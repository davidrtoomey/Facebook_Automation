...
class SQLiteStateManager:
    ...
    def init_database(self):
        ...
        if "unavailable" not in columns:
            try:
                cursor.execute(
                    "ALTER TABLE listings ADD COLUMN unavailable BOOLEAN DEFAULT FALSE"
                )
                logger.info("Added 'unavailable' column to listings table")
            except Exception as e:
                logger.warning(f"Could not add unavailable column: {e}")

        if "relevant" not in columns:
            try:
                cursor.execute(
                    "ALTER TABLE listings ADD COLUMN relevant BOOLEAN DEFAULT TRUE"
                )
                logger.info("Added 'relevant' column to listings table")
            except Exception as e:
                logger.warning(f"Could not add relevant column: {e}")
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_conversation ON listings(conversation_status)"
        )
    ...
    def get_unmessaged_listings(
        ...
        query = """
            ...
            WHERE messaged = FALSE 
              AND (unavailable IS FALSE OR unavailable IS NULL)
              AND (relevant IS TRUE OR relevant IS NULL)
        ...
        for row in cursor.fetchall():
            ...
            try:
                listing["condition_hints"] = json.loads(
                    listing["condition_hints"] or "[]"
                )
            except:
    ...
    def get_cached_offer_price(self, cache_key):
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT offer_data FROM pricing_cache WHERE cache_key = ? AND created_at > DATETIME('now', '-30 days')",
            (cache_key,),
        )
        result = cursor.fetchone()
        if result:
    ...
    def cache_offer_price(self, cache_key, result):
        ...
        self.conn.execute(
            "INSERT OR REPLACE INTO pricing_cache (cache_key, offer_data) VALUES (?, ?)",
    ...
    def get_listing_stats(self) -> Dict[str, int]:
        ...
        try:
            ...
            # Unmessaged listings
            stats["unmessaged_listings"] = (
                stats["total_listings"] - stats["messaged_listings"]
            )
    ...
    def cleanup_old_cache(self, days: int = 30):
        ...
        try:
            ...
            if cursor.fetchone()[0] > 10000:
                cursor.execute(
                    "DELETE FROM product_cache ORDER BY created_at ASC LIMIT ?",
                    (deleted - 10000,),
                )
                self.conn.commit()
    ...
    def is_migrated(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='migration_flag'"
        )
    ...
    def export_to_json(self, json_path: str = "listings.json") -> int:
        """Export all listings from SQLite to JSON for backward compatibility."""
        try:
            cursor = self.conn.cursor()
            ...
            for row in cursor.fetchall():
                ...
                # Parse condition_hints JSON
                try:
                    if listing.get("condition_hints"):
                        listing["condition_hints"] = json.loads(
                            listing["condition_hints"]
                        )
                    else:
            ...
            logger.info(f"✅ Exported {len(listings)} listings to {json_path}")
            return len(listings)

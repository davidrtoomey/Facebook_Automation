...
class AutomationRunner:
    ...
    def __init__(self):
        ...
        self.scripts_dir = (
            self.project_root
        )  # Main project directory contains the scripts
    ...
    def _kill_existing_automation_processes(self):
        ...
        try:
                ...
                if "python" in line.lower() and any(
                    ...
                    if len(parts) > 1:
                        try:
                            ...
                            print(
                                f"[STARTUP CLEANUP] Found existing automation process: PID {pid} - {process_name}"
                            )
            ...
            if killed_processes:
                ...
                print(
                    f"[STARTUP CLEANUP] Cleaned up {len(killed_processes)} existing automation processes"
                )
            else:
                print("[STARTUP CLEANUP] No existing automation processes found")

            # Also kill any zombie browser processes on startup
            try:
                import psutil
                ...
                for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                    try:
                        ...
                        if "chrome" in name or "chromium" in name:
                            ...
                            if any(
                                "browseruse" in str(arg) or "agent" in str(arg)
                                for arg in cmdline
                            ):
                                print(
                                    f"[STARTUP CLEANUP] Terminating zombie browser process {proc.info['pid']}"
                                )
                                proc.terminate()
                ...
                print(
                    "[STARTUP CLEANUP] psutil not installed, skipping zombie browser cleanup"
                )
            except Exception as e:
                print(f"[STARTUP CLEANUP] Error cleaning zombie browsers: {e}")
    ...
    def _initialize_automation_state(
        self, config: Dict[str, Any], automation_type: str = "full_automation"
    ) -> AutomationState:
        ...
        try:
            ...
            print(
                f"[STATE DEBUG] - Has existing products: {bool(existing_state.current_cycle_products)}"
            )
            print(
                f"[STATE DEBUG] - Previous products: {existing_state.current_cycle_products}"
            )
            print(f"[STATE DEBUG] - Current products: {current_products}")
            print(
                f"[STATE DEBUG] - Products match: {existing_state.current_cycle_products == current_products}"
            )
            print(
                f"[STATE DEBUG] - Previous automation type: {existing_state.automation_type}"
            )
            print(f"[STATE DEBUG] - Current automation type: {automation_type}")
            print(
                f"[STATE DEBUG] - Automation type match: {existing_state.automation_type == automation_type}"
            )

            if (
                not existing_state.current_cycle_products
                or existing_state.current_cycle_products != current_products
                or existing_state.automation_type != automation_type
            ):
                print(
                    f"[STATE] Configuration changed or first run, resetting automation state"
                )
                print(
                    f"[STATE] Previous products: {existing_state.current_cycle_products}"
                )
                print(f"[STATE] Current products: {current_products}")

                new_state = self.config_manager.reset_automation_state(
                    current_products, automation_type
                )
                return new_state

            print(
                f"[STATE] Loaded existing automation state - last completed: {existing_state.last_completed_product_index}"
            )
            ...
            return self.config_manager.reset_automation_state(
                self._get_product_names_from_config(config), automation_type
            )

    def _save_product_completion(
        self, state: AutomationState, completed_product_index: int
    ):
        ...
        try:
            ...
            product_name = (
                state.current_cycle_products[completed_product_index]
                if completed_product_index < len(state.current_cycle_products)
                else "Unknown"
            )
            print(
                f"[STATE] Saved completion of product '{product_name}' (index {completed_product_index})"
            )

        except Exception as e:
            print(f"[STATE] Error saving product completion: {e}")

    async def _send_result_update(
        self, progress_callback: Callable, result: Dict[str, Any]
    ):
    ...
    async def _update_pricing_data(self, progress_callback: Callable):
        ...
        try:
            ...
            pricing_script = os.path.join(
                self.project_root, "utils", "get_pricing_data.py"
            )
            ...
            if os.path.exists(pricing_script):
                ...
                if process.returncode == 0:
                    await progress_callback(
                        "running", 3, "✅ Pricing data updated successfully"
                    )
                    ...
                    await progress_callback(
                        "running",
                        3,
                        "⚠️ Pricing update had issues, continuing with existing data",
                    )
                    print(f"⚠️ Pricing update stderr: {stderr.decode()}")
            else:
                await progress_callback(
                    "running", 2, "⚠️ Pricing script not found, using existing data"
                )
            ...
            await progress_callback(
                "running",
                2,
                f"⚠️ Pricing update failed: {str(e)}, continuing with existing data",
            )
            print(f"❌ Error updating pricing data: {e}")

    async def run(
        self, config: Dict[str, Any], progress_callback: Callable
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            automation_state = self._initialize_automation_state(
                config, "full_automation"
            )

            # Debug: Log the products being processed
            print(
                f"[DEBUG] Processing {len(search_products)} products: {[p.get('name') if isinstance(p, dict) else p for p in search_products]}"
            )
            ...
            if start_index > 0:
                await progress_callback(
                    "running",
                    5,
                    f"Resuming automation from product {start_index + 1}/{total_products}: '{automation_state.current_cycle_products[start_index]}'",
                )
            else:
                await progress_callback(
                    "running",
            ...
            if start_index + len(range(start_index, total_products)) >= total_products:
                ...
                await progress_callback(
                    "running",
                    95,
                    "Full automation cycle completed, resetting for next cycle...",
                )
                automation_state = automation_state.reset_for_new_cycle(
                    automation_state.current_cycle_products
                )
                self.config_manager.save_automation_state(automation_state)
                await progress_callback(
                    "running",
                    100,
                    f"Automation cycle completed! Processed {total_products} products. Ready for next cycle.",
                )
            else:
                await progress_callback(
                    "running",
                    100,
                    f"Automation batch completed! Processed {len(range(start_index, total_products))} products.",
                )
            ...
            if "automation_state" in locals() and "product_index" in locals():
                print(
                    f"[STATE] Saving progress due to error at product index {product_index}"
                )
                self._save_product_completion(
                    automation_state, product_index - 1
                )  # Save previous completed index
        ...
        try:
            ...
            if start_index > 0:
                await progress_callback(
                    "running",
                    5,
                    f"Resuming offer agent from product {start_index + 1}/{total_products}: '{automation_state.current_cycle_products[start_index]}'",
                )
            else:
                await progress_callback(
                    "running", 5, "Starting offer agent for existing listings..."
                )
            ...
            for product_index in range(start_index, total_products):
                ...
                await progress_callback(
                    "running",
                    base_progress + 10,
                    f"Processing offers for '{search_product}'...",
                )
            ...
            if start_index + len(range(start_index, total_products)) >= total_products:
                ...
                await progress_callback(
                    "running",
                    95,
                    f"Offer agent cycle completed! Processed {total_products} products. Ready for next cycle.",
                )
            else:
                await progress_callback(
                    "running",
                    95,
                    f"Offer agent batch completed! Processed {len(range(start_index, total_products))} products.",
                )
    ...
    async def run_conversations_only(
        self, config: Dict[str, Any], progress_callback: Callable
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if start_index > 0:
                await progress_callback(
                    "running",
                    5,
                    f"Resuming conversation agent from product {start_index + 1}/{total_products}: '{automation_state.current_cycle_products[start_index]}'",
                )
            else:
                await progress_callback(
                    "running", 5, "Starting conversation agent for existing messages..."
                )
            ...
            for product_index in range(start_index, total_products):
                ...
                await progress_callback(
                    "running",
                    base_progress + 10,
                    f"Formatting and deduplicating conversation data...",
                )
                await self._fix_duplicates(
            ...
            if start_index + len(range(start_index, total_products)) >= total_products:
                ...
                await progress_callback(
                    "running",
                    95,
                    f"Conversation agent cycle completed! Processed {total_products} products. Ready for next cycle.",
                )
            else:
                await progress_callback(
                    "running",
                    95,
                    f"Conversation agent batch completed! Processed {len(range(start_index, total_products))} products.",
                )
    ...
    async def run_scrape_listings_only(
        self, config: Dict[str, Any], progress_callback: Callable
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            await progress_callback(
                "running", 5, f"Starting listing scraper for: {search_term}"
            )
            ...
            await progress_callback(
                "running", 15, f"Scraping listings for '{search_term}'..."
            )

            # Run only the listing search script
            await self._run_listing_search(
                search_term, search_keywords, config, progress_callback, 15
            )
            ...
            await progress_callback(
                "running", 70, f"Deduplicating listings for '{search_term}'..."
            )
            ...
            await progress_callback(
                "running", 95, f"Listing scraper completed for '{search_term}'"
            )
    ...
    def _force_cleanup_all_processes(self):
                    ...
                    if "python" in proc.info["name"].lower():
                        for script in automation_scripts:
                            if any(script in arg for arg in cmdline):
                                print(
                                    f"[FORCE KILL] Terminating {script} process (PID: {proc.info['pid']})"
                                )
                                try:
        ...
        # Also kill any Chrome/Chromium processes that might be hanging around
        try:
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if "chrome" in name or "chromium" in name:
                        ...
                        if any(
                            ...
                            print(
                                f"[FORCE KILL] Terminating {name} browser process (PID: {proc.info['pid']})"
                            )
                            proc.terminate()
                            killed_count += 1
                except (psutil.NoSuchProcess, psutil.AccessDenied):
        ...
        for product in search_products:
            ...
            if isinstance(product, dict):
                ...
                required_fields = [
                    "base_offer_unlocked",
                    "base_offer_locked",
                    "base_offer_unlocked_damaged",
                    "base_offer_locked_damaged",
                ]
    ...
    async def _run_listing_search(
        ...
        try:
            ...
            await progress_callback(
                "console", 0, f"DEBUG: Script exists: {os.path.exists(script_path)}"
            )
            await progress_callback(
                "console", 0, f"DEBUG: Project root: {self.project_root}"
            )
            await progress_callback(
                "console", 0, f"DEBUG: Python executable: {sys.executable}"
            )
            await progress_callback(
                "console",
                0,
                f"DEBUG: Environment vars set: SEARCH_PRODUCT={env.get('SEARCH_PRODUCT')}",
            )

            await progress_callback(
                "running",
                current_progress,
                f"Starting listing search for '{search_product}'...",
            )
            ...
            stdout_lines, stderr_lines = await self._run_process_with_streaming(
            ...
            if process.returncode != 0:
                ...
                raise Exception(
                    f"Listing search failed for '{search_product}': {error_msg}"
                )

            await progress_callback(
                "running",
                current_progress + 10,
                f"Listing search completed for '{search_product}'",
            )
            ...
            raise Exception(
                f"Failed to run listing search for '{search_product}': {str(e)}"
            )
    ...
    async def _fix_duplicates(
        ...
        try:
            ...
            # Log debug info
            await progress_callback(
                "console", 0, f"DEBUG: Fix duplicates script path: {script_path}"
            )
            await progress_callback(
                "console",
                0,
                f"DEBUG: Fix duplicates script exists: {os.path.exists(script_path)}",
            )

            await progress_callback(
                "running",
                current_progress,
                f"Starting deduplication for '{search_product}'...",
            )
            ...
            try:
                ...
                await progress_callback(
                    "console", 0, "Running deduplication function..."
                )
                ...
                try:
                    ...
                    deduplicated = fix_duplicates(
                        "listings.json", None, auto_replace=True
                    )

                    await progress_callback(
                        "console", 0, f"Deduplication completed successfully"
                    )
            ...
            await progress_callback(
                "running",
                current_progress + 10,
                f"Deduplication completed for '{search_product}'",
            )

        except Exception as e:
            raise Exception(
                f"Failed to run deduplication for '{search_product}': {str(e)}"
            )

    async def _extract_marketplace_urls(
        self,
        search_product: str,
        config: Dict[str, Any],
        progress_callback: Callable,
        ...
        try:
            ...
            env["GEMINI_API_KEY"] = config["gemini_api_key"]
            env["SEARCH_PRODUCT"] = search_product
            env["HEADLESS_CONVERSATIONS"] = str(
                config.get("headless_conversations", False)
            ).lower()
            ...
            await progress_callback(
                "running",
                current_progress,
                f"Starting marketplace URL extraction for '{search_product}'...",
            )
            ...
            self.running_processes.append(process)
            print(
                f"[PROCESS] Started process {process.pid} for marketplace URL extraction"
            )
            ...
            if process.returncode != 0:
                ...
                raise Exception(
                    f"URL extraction failed for '{search_product}': {error_msg}"
                )

            await progress_callback(
                "running",
                current_progress + 10,
                f"URL extraction completed for '{search_product}'",
            )

        except Exception as e:
            raise Exception(
                f"Failed to extract marketplace URLs for '{search_product}': {str(e)}"
            )
    ...
    async def _send_offers(
        ...
        try:
            ...
            env["GEMINI_API_KEY"] = config["gemini_api_key"]
            env["SEARCH_PRODUCT"] = search_product
            env["BASE_OFFER_UNLOCKED"] = str(
                product_pricing.get("base_offer_unlocked", 300)
            )
            env["BASE_OFFER_LOCKED"] = str(
                product_pricing.get("base_offer_locked", 250)
            )
            env["BASE_OFFER_UNLOCKED_DAMAGED"] = str(
                product_pricing.get("base_offer_unlocked_damaged", 150)
            )
            env["BASE_OFFER_LOCKED_DAMAGED"] = str(
                product_pricing.get("base_offer_locked_damaged", 100)
            )
            ...
            await progress_callback(
                "running",
                current_progress,
                f"Starting offer agent for '{search_product}'...",
            )
            ...
            if process.returncode != 0:
                ...
                raise Exception(
                    f"Offer agent failed for '{search_product}': {error_msg}"
                )

            await progress_callback(
                "running",
                current_progress + 15,
                f"Offer agent completed for '{search_product}'",
            )
            ...
            if os.path.exists(listings_file):
                ...
                for listing in listings:
                    if isinstance(listing, dict):
                        ...
                        if listing.get("messaged", False) and listing.get(
                            "messaged_at"
                        ):
                            try:
                                messaged_time = datetime.fromisoformat(
                                    listing["messaged_at"]
                                )
                        ...
                        is_current_product = (
                            search_product.lower() in listing_product.lower()
                            or not listing_product
                        )
                        ...
                        if is_current_product or messaged_recently:
                            result = {
                                ...
                                "timestamp": datetime.now().isoformat(),
                                "offer_amount": product_pricing.get(
                                    "base_offer_unlocked", 300
                                ),
                                "type": "offer",
                            ...
                            if messaged_recently:
                                offers_sent_count += 1
                                await self._send_result_update(
                                    progress_callback, result
                                )
                                print(
                                    f"[STATS] Sent WebSocket update for offer #{offers_sent_count}"
                                )
    ...
    async def _get_marketplace_urls(
        self,
        search_product: str,
        config: Dict[str, Any],
        progress_callback: Callable,
        ...
        try:
            ...
            await progress_callback(
                "running",
                current_progress,
                f"🚀 Starting URL extraction - launching browser...",
            )
            ...
            if process.returncode == 0:
                await progress_callback(
                    "running",
                    current_progress + 5,
                    f"Successfully extracted marketplace URLs",
                )
                print(
                    f"[SUCCESS] Marketplace URL extraction completed for '{search_product}'"
                )
            else:
                ...
                await progress_callback(
                    "error",
                    current_progress,
                    f"Marketplace URL extraction failed: {error_message}",
                )
    ...
    async def _handle_conversations(
        ...
        try:
            ...
            env["GEMINI_API_KEY"] = config["gemini_api_key"]
            env["SEARCH_PRODUCT"] = search_product
            env["ENABLE_NEGOTIATION"] = str(
                config.get("enable_negotiation", True)
            ).lower()
            env["PRICE_FLEXIBILITY"] = str(config.get("price_flexibility", 20))
            ...
            await progress_callback(
                "running",
                current_progress,
                f"Starting conversation agent for '{search_product}'...",
            )
            ...
            if process.returncode != 0:
                error_msg = (
                    "\n".join(stderr_lines)
                    if stderr_lines
                    else "Conversation agent failed"
                )
                raise Exception(
                    f"Conversation agent failed for '{search_product}': {error_msg}"
                )

            await progress_callback(
                "running",
                current_progress + 15,
                f"Conversation agent completed for '{search_product}'",
            )
            ...
            if os.path.exists(messages_file):
                ...
                conversations = (
                    messages_data.get("conversations", [])
                    ...
                    if isinstance(conversation, dict):
                        ...
                        result = {
                            ...
                            "message": conversation.get(
                                "last_message", "Conversation processed"
                            ),
                        ...
                        if conv_status in active_statuses:
                            ...
                            print(
                                f"[STATS] Sent WebSocket update for active negotiation: {conv_status}"
                            )
                        elif conv_status == "deal_closed":
                            await self._send_result_update(progress_callback, result)
                ...
                print(
                    f"[STATS] Processed {len(conversations)} conversations, sent WebSocket updates for active negotiations"
                )
            ...
            raise Exception(
                f"Failed to handle conversations for '{search_product}': {str(e)}"
            )
    ...
    async def test_configuration(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results = {
                ...
                "pricing_valid": self._validate_product_pricing(
                    config.get("search_products", [])
                ),
                "scripts_available": self._check_scripts_available(),
            }
